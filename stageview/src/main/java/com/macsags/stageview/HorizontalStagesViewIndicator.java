...
public class HorizontalStagesViewIndicator extends View {
    ...
    private List<StageBean> mStepBeanList;//当前有几部流程    there are currently few step
    ...
    public HorizontalStagesViewIndicator(Context context) {
        this(context, null);
    }

    public HorizontalStagesViewIndicator(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public HorizontalStagesViewIndicator(Context context, AttributeSet attrs, int defStyle) {
    ...
    private void init() {
        ...
        mLinePadding = 0.70f * defaultStepIndicatorNum;

        mCompleteIcon = ContextCompat.getDrawable(getContext(), R.drawable.yiwancheng);//已经完成的icon
        mAttentionIcon = ContextCompat.getDrawable(getContext(), R.drawable.jinxingzhong);//正在进行的icon
    ...
    protected synchronized void onDraw(Canvas canvas) {
        ...
        for (int i = 0; i < mCircleCenterPointPositionList.size() - 1; i++) {
            ...
            if (i <= mComplectingPosition && mStepBeanList.get(0).getState() != StageBean.STEP_UNDO)//判断在完成之前的所有点
            {
        ...
        for (int i = 0; i < mCircleCenterPointPositionList.size(); i++) {
            ...
            StageBean stepsBean = mStepBeanList.get(i);
    ...
    public void setStepNum(List<StageBean> stepsBeanList) {
        ...
        if (mStepBeanList != null && mStepBeanList.size() > 0) {
            for (int i = 0; i < mStepNum; i++) {
                StageBean stepsBean = mStepBeanList.get(i);
                {
                    if (stepsBean.getState() == StageBean.STEP_COMPLETED) {
